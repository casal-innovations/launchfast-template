...
export async function createStripePlans() {
	...
	if (plans.length > 0) {
		console.log('🎉 Plans have already been seeded.')
		return true
	}
		...
		async ({ id, name, description, /* features, */ limits, prices }) => {
			...
			try {
				const stripePrices = await Promise.all(
					pricesByInterval.map(price => createStripePrice(id, price)),
				)
	...
	try {
		// Create Stripe products and store them in database
		const seededProducts = await Promise.all(seedProducts)
		console.log(`📦 Stripe Products have been successfully created.`)
		...
		console.log(
			'🎉 Visit: https://dashboard.stripe.com/products to see your products.',
		)
	} catch (error) {
		throw new Error(
			`Failed to create Stripe product/customer-portal\nError: ${error}`,
		)
	}
}
