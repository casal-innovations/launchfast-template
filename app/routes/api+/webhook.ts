 ...
 */
async function getStripeEvent(request: Request, stripe: Stripe) {
	try {
		...
		const event = stripe.webhooks.constructEvent(
			payload,
			signature,
			ENDPOINT_SECRET || 'Invalid *_STRIPE_WEBHOOK_ENDPOINT',
...
export async function action({ request }: ActionFunctionArgs) {
	...
	const event = await getStripeEvent(request, stripe)
